...
/*  這是viewday讀取檔案輸出的方式 
	...
	fseek(fout,138,SEEK_SET); //尋找第一筆資料的位置 
		
	for(i=0;i<10;i++)
	{
		fread(date,2,1,fout); //從file讀取一組長度為2的資料到date裡 
		
		sscanf(date, "%d", &temp); //char array 轉換成 int 
		temp=atoi(data);也可以 
		
		if(temp==n)
		{
			fseek(fout,138+i*(sizeof(record)),SEEK_SET); //尋找第i筆資料的開頭 
				
			fread(record,138,1,fout); //從file讀取一行資料到record裡 
			printf("the record at %d\n-> %s\n\n",n,record);
			return 0;
		}
		fseek(fout,(sizeof(record)-2),SEEK_CUR); //尋找下一筆資料的位置
	}
...
/*	這是viewweek讀取檔案輸出的方式 
	...
	for(i=0;i<10;i++)
	{
		...
		if(temp>=n && temp<n+7)
		{
			fseek(fout,144+i*(sizeof(record)+4),SEEK_SET); //尋找第i筆資料的開頭 
			
		//	fread(temprecord,138,1,fout); //從file讀取一行資料到temprecord裡 
		//	sscanf(date, "%d", &temp); //char array 轉換成 int 

			fread(viewrecord[i].name,19,1,fout); //從file讀取一行資料到viewrecord[i].name裡 
			fseek(fout,1,SEEK_CUR); 

			fread(viewrecord[i].location,29,1,fout); //從file讀取一行資料到viewrecord[i].location裡 
			fseek(fout,1,SEEK_CUR); 

			fread(viewrecord[i].thing,79,1,fout); //從file讀取一行資料到viewrecord[i].thing裡 
			fseek(fout,1,SEEK_CUR);
			
			fseek(fout,4,SEEK_CUR); //尋找下一筆資料的位置
			hasrecord=true;
